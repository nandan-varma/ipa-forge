...
description: "forge analysis: class-dump, strings, symbols, security posture, and version diffing for any decrypted IPA."
---
...
see [Architecture](/docs/architecture) for how the two relate. Every
command accepts `--app-dir <Payload/App.app>` in place of `--ipa` to skip
re-extraction while iterating, same as `forge hooks`.

<Callout type="warn" title="Scope boundary">
...
<TypeTable
  type={{
    ...
    "--output <path>, -o": { type: "path", description: "Write to a file instead of stdout" },
  }}
/>
...
<TypeTable
  type={{
    "--ipa <path>": { type: "path", description: "Input .ipa (omit when --app-dir is given)" },
    "--app-dir <path>": { type: "path", description: "Already-extracted Payload/<App>.app, instead of --ipa" },
    "--min-len <n>": { type: "number", default: "4", description: "Minimum printable-ASCII run length to report" },
    "--search <regex>": { type: "string", description: "Only strings matching this regex" },
    "--binary <substring>": { type: "string", description: "Restrict to one executable (bundle-relative substring match)" },
  }}
/>
...
<TypeTable
  type={{
    "--ipa <path>": { type: "path", description: "Input .ipa (omit when --app-dir is given)" },
    "--app-dir <path>": { type: "path", description: "Already-extracted Payload/<App>.app, instead of --ipa" },
    "--binary <substring>": { type: "string", default: "main executable", description: "Which executable to inspect (a framework, extension, or standalone dylib), by bundle-relative substring" },
    "--arch <name>": { type: "string", description: "Architecture slice; required if the target is a universal binary" },
  }}
/>
...
<TypeTable
  type={{
    "--ipa <path>": { type: "path", description: "Input .ipa (omit when --app-dir is given)" },
    "--app-dir <path>": { type: "path", description: "Already-extracted Payload/<App>.app, instead of --ipa" },
    "--binary <substring>": { type: "string", default: "main executable", description: "Which executable to inspect, by bundle-relative substring" },
    "--arch <name>": { type: "string", description: "Architecture slice; required if the target is a universal binary" },
  }}
/>
...
<TypeTable
  type={{
    "--old <path>": { type: "path", description: "Previous-version .ipa (omit when --old-app-dir is given)" },
    "--new <path>": { type: "path", description: "New-version .ipa (omit when --new-app-dir is given)" },
...
<Callout type="info" title="forge analysis diff vs. forge hooks diff">
This is **broader but shallower** than `forge hooks diff`
(see [Usage](/docs/usage#forge-hooks-diff)): `forge
...
module allowed to do that ([Architecture](/docs/architecture)'s hard
constraint) — a real entitlements diff belongs in that subsystem, not in
this read-only, signing-independent package.
...
`http://127.0.0.1:8765/analysis`. Unlike the main patcher page, `/analysis`
needs no patch-set setup and works on any IPA — upload it and pick an
action. Strings output is capped at 2000 matches in the browser (the CLI has
no such cap). Single-user, local-only, same as the
patch-flow page — see [Usage](/docs/usage#the-web-gui).

## Related

<Cards>
  <Card title="Architecture" description="The machO/objc.py analysis engine hooks/ and analysis/ share" href="/docs/architecture" />
  <Card title="Usage" description="forge hooks and the rest of the CLI" href="/docs/usage" />
</Cards>
