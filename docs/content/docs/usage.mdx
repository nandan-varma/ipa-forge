...
- **macOS** with Xcode Command Line Tools (`xcode-select -p` prints a path).
  ...
  Linux (see [`extensibility`](/docs/extensibility)).
- **Python 3.11+**.
...
See the [Patch Definition Reference](/docs/patch-reference) — every operation
...
[`altstore-device-testing`](/docs/altstore-device-testing).
...
A concrete patch set's `PLAYBOOK.md` (see [`adding-an-app`](/docs/adding-an-app))
...
- [`adding-an-app`](/docs/adding-an-app) — port a new app
- [`adding-a-feature`](/docs/adding-a-feature) — add a feature
...
- [`troubleshooting`](/docs/troubleshooting) — errors
