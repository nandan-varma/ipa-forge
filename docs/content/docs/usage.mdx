...
<Callout type="warn" title="Signing requires macOS">
Signing shells out to Apple's own `codesign`/`security` and is never
reimplemented, so it requires macOS with Xcode Command Line Tools
(`xcode-select -p` prints a path). Everything up to and including patch
dry-run also works on Linux — see [Extensibility](/docs/extensibility).
</Callout>
...
forge hooks extract --ipa App.ipa --search "Config"
...
Every command re-extracts the full IPA (seconds to tens of seconds on a large
app) unless you point it at an already-extracted bundle instead — pass
...
<TypeTable
  type={{
    "IPA": { type: "file", required: true, description: "The .ipa to patch" },
    "Patch definition": { type: "file", required: true, description: "A single .yaml/.yml/.json, or a zip of the patch directory (definition + its assets/ folder) when you use resource_replace/resource_add. The zip must contain exactly one top-level definition file; its assets/ resolve relative to it." },
    "Provisioning profile(s)": { type: "file[]", description: "Select several to sign each extension/watch app with its own profile" },
    "Signing identity": { type: "string", description: "SHA-1 or name substring" },
    "Dry run switch": { type: "boolean", description: "Validates only, no signing; the profile/identity fields gray out. Not needed for dry runs." },
  }}
/>

The **result card** shows "Patched"/"Dry Run OK", the bundle/version, a
