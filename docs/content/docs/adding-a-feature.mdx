---
title: Adding a feature to a hook dylib
...
a reason not to (e.g. a behavior change users might not want) — **with one
standing exception**: a feature built via reverse engineering with no
reference source to check against and no on-device confirmation yet
defaults **OFF** and is labeled beta in the settings row. Flip it to
default-ON once a device pass confirms it works.
...
- Prefer pure ObjC-runtime swizzling. **No C-level rebinding** (fishhook-style)
  unless unavoidable; if you must, defer it past launch and guard every
  resolved original pointer.
...
`hooks:` block yet. `--dry-run` alone won't catch this — the gap is
...
submodule itself**, if the patch set lives in a separate git repo.
...
- [`adding-an-app`](/docs/adding-an-app) — end-to-end porting
